    ...
    ARCH_SWITCH(params.arch, Arch, [&] {
        SOFTCAP_SWITCH(params.softcap > 0.f, Has_softcap, [&] {
            if (!params.is_bf16) {
                #ifndef FLASHATTENTION_DISABLE_FP16
                #ifndef FLASHATTENTION_DISABLE_HDIM64
                if (params.d <= 64) { return run_mha_bwd_<Arch, cutlass::half_t, 64, 64, Has_softcap>(params, stream); }
                #endif
                #ifndef FLASHATTENTION_DISABLE_HDIM96
                if (params.d <= 96) { return run_mha_bwd_<Arch, cutlass::half_t, 96, 96, Has_softcap>(params, stream); }
                #endif
                #ifndef FLASHATTENTION_DISABLE_HDIM128
                if (params.d <= 128) { return run_mha_bwd_<Arch, cutlass::half_t, 128, 128, Has_softcap>(params, stream); }
                #endif
                #ifndef FLASHATTENTION_DISABLE_HDIM192
                if (params.d <= 192) {
                    if (params.dv == 128) {
                        return run_mha_bwd_<Arch, cutlass::half_t, 192, 128, Has_softcap>(params, stream);
                    } else if (params.dv == 192) {
                        return run_mha_bwd_<Arch, cutlass::half_t, 192, 192, Has_softcap>(params, stream);
                    }
                }
                #endif
                #ifndef FLASHATTENTION_DISABLE_HDIM256
                if (params.d <= 256) { return run_mha_bwd_<Arch, cutlass::half_t, 256, 256, Has_softcap>(params, stream); }
                #endif
                #else
                TORCH_CHECK(false, "This flash attention build does not support FP16.");
                #endif
                ...
                #ifndef FLASHATTENTION_DISABLE_HDIM64
                if (params.d <= 64) { return run_mha_bwd_<Arch, cutlass::bfloat16_t, 64, 64, Has_softcap>(params, stream); }
                #endif
                #ifndef FLASHATTENTION_DISABLE_HDIM96
                if (params.d <= 96) { return run_mha_bwd_<Arch, cutlass::bfloat16_t, 96, 64, Has_softcap>(params, stream); }
                #endif
                #ifndef FLASHATTENTION_DISABLE_HDIM128
                if (params.d <= 128) { return run_mha_bwd_<Arch, cutlass::bfloat16_t, 128, 128, Has_softcap>(params, stream); }
                #endif
                #ifndef FLASHATTENTION_DISABLE_HDIM192
                if (params.d <= 192) { 
                    if (params.dv == 128) {
                        return run_mha_bwd_<Arch, cutlass::bfloat16_t, 192, 128, Has_softcap>(params, stream);     
                    } else if (params.dv == 192) {
                        return run_mha_bwd_<Arch, cutlass::bfloat16_t, 192, 192, Has_softcap>(params, stream);     
                    }
                }
                #endif
                #ifndef FLASHATTENTION_DISABLE_HDIM256
                if (params.d <= 256) { return run_mha_bwd_<Arch, cutlass::bfloat16_t, 256, 256, Has_softcap>(params, stream); }
                #endif
            }
        });
    });
...
std::vector<at::Tensor> mha_bwd(
    ...
    int const max_headdim = get_max_headdim();
    TORCH_CHECK(head_size <= max_headdim && head_size_v <= max_headdim, "FlashAttention backward only supports head dimension at most " + std::to_string(max_headdim));
    TORCH_CHECK(head_size_v <= head_size, "the head_size_v should not be larger than head_size");
    TORCH_CHECK(num_heads % num_heads_k == 0, "Number of heads in key/value must divide number of heads in query");
